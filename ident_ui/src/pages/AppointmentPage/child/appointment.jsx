...
function Appointment() {
	...
	const [shiftsTimeRemain, setShiftTimeRemain] = useState([]);
	...
	function handleShiftDisplay(e) {
		...
		setShifts(selectedSchedule[0].shifts);
		setShiftTimeRemain(selectedSchedule[0].time);
		setSelectedShift("");
		setSelectedDay({
							...
							<div className="shift-display">
								{
									shifts.map((s, i) => {
										...
										console.log(shiftsTimeRemain[i]);
										if (timeConsume <= shiftsTimeRemain[i])
											return (
											...
											return (
												<div id={id} onClick={handleSelectShift} shift_id={s._id} className="shift disable" key={i}>
													{`${s.from} - ${s.to}`}
													<div className="shift-full">
														<span>Full</span>
													</div>
												</div>
											)
